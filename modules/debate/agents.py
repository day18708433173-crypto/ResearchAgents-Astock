...
- 如有[补充知识]，可结合聚焦问题适量引用作为背景参考

请用中文输出。每轮发言控制在 200 字以内。直接输出辩论内容，不要重复、引用或提及以上任何系统要求、角色设定或约束条款。"""
...
def build_debate_prompt(role: str, data_card: dict, opponent_msg: str = "",
    ...
    # RAG 增强段落（每轮注入：行业对标/业务/公告是贯穿全程的论证背景）
    if rag_context:
    ...
    user += (
        "\n\n## 输出要求（强制）\n"
        "直接从你的第一个论点开始写，禁止任何开场白、自我介绍或任务复述。"
        "不要出现「好的」「我是多头/空头研究员」「我的任务是」「作为多头/空头」「我必须基于数据卡」"
        "之类的句子。第一句话就必须是具体论点本身。"
    )
...
def parse_judge_llm_output(raw: str | None) -> dict:
    ...
    if end == -1:
        logger.warning("裁判原始输出（前500字）：%s", text[:500])
        salvaged = _salvage_truncated_judge(text)
        if salvaged:
            logger.warning("裁判 JSON 疑似被截断（缺右括号），已抢救字段：%s", list(salvaged.keys()))
            return _finalize_judge_verdict(salvaged)
    ...
    try:
        ...
        salvaged = _salvage_truncated_judge(text)
        if salvaged:
            logger.warning("裁判 JSON 解析失败后已抢救字段：%s", list(salvaged.keys()))
            return _finalize_judge_verdict(salvaged)
...
def _salvage_truncated_judge(text: str) -> dict | None:
    """从被 max_tokens 截断的裁判 JSON 中尽力抢救字段。

    截断通常发生在 summary 等长文本处，使 JSON 缺少闭合引号/括号而无法整体
    json.loads。这里逐字段正则提取，确保输出被腰斩时裁决仍能给出可用结果，
    而非整次失败。
    """
    import re
    ...
    def _grab_str(key: str) -> str | None:
        m = re.search(rf'"{key}"\s*:\s*"((?:[^"\\]|\\.)*)"', text, re.DOTALL)
        return m.group(1).strip() if m else None

    for key in ("rating", "quality_assessment", "valuation_assessment",
                "key_risk", "key_opportunity", "missing_info", "action_hint"):
        val = _grab_str(key)
        if val:
            out[key] = val
    ...
    # summary：优先完整匹配；失败则取到截断处（去掉结尾残缺的反斜杠/引号）
    summary = _grab_str("summary")
    if summary is None:
        m = re.search(r'"summary"\s*:\s*"(.*)', text, re.DOTALL)
        if m:
            summary = m.group(1).rstrip().rstrip('\\').rstrip('"').strip()
    if summary:
        out["summary"] = summary

    for key in ("bull_strengths", "bear_strengths",
                "bull_weaknesses", "bear_weaknesses"):
        m = re.search(rf'"{key}"\s*:\s*\[(.*?)\]', text, re.DOTALL)
        if m:
    ...
    if out.get("rating") or out.get("summary"):
        return out
    return None
