/**
 * SSE 裁判裁决代理：转发 x-jh-llm-* 自定义头到后端
 */
...
export async function GET(request: Request) {
  ...
  if (!ticker || !debate_id) {
    return new Response(JSON.stringify({ error: '缺少 ticker 或 debate_id 参数' }), {
      status: 400,
      headers: { 'Content-Type': 'application/json' },
    });
  }

  const targetUrl = `${BACKEND_URL}/api/debate/judge?${searchParams.toString()}`;
  ...
  try {
    ...
    console.error('[Judge SSE Proxy] 连接后端失败:', error);
